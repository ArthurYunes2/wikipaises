  ...
  useEffect(() => {
    ...
    if (search.trim()) {
      ...
      filtered = filtered.filter((c) => {
        const nameMatch = c.name?.common?.toLowerCase().includes(q)
        const capitalMatch = c.capital?.[0]?.toLowerCase().includes(q)
        const languageMatch = c.languages
          ? Object.values(c.languages).some((l) => l.toLowerCase().includes(q))
          : false
        const currencyMatch = c.currencies
          ...
          : false
        return nameMatch || capitalMatch || languageMatch || currencyMatch
      })
    }
